...
## More Word Categories: Morality and Politeness

### Morality (by Moral Foundations Dictionary)
``` {r MFD, fig.align="center", fig.dim=c(12,12)}
load("MFD.Rdata")
library(ggradar)
...
Output_MFD %>%
  ...
  ggradar(grid.max = 3,  group.line.width = 1, group.point.size = .6, axis.label.size = 5) + 
    labs(title = "Moral Concerns by Topic")
...
Output_MFD %>%
  ...
  summarize (Care = mean(Care),
             ...
             Authority = mean(Authority),
             Sanctity = mean(Sanctity)) %>%
  ggradar(grid.max = 1.5,group.line.width = 1, group.point.size = .6, axis.label.size = 5) + 
  labs(title = "Moral Concerns by Medium")


```

**Comment**: In the Drug topic `Sanctity` is more salient because of the words related to health. Similarly, words related to fairness - e.g., retribution, justice, inequality - are more salient in the Reparations topic. Now, we can see that text conversations were definitely more moralized than audio conversations, in all domains. It doesn't seem to be an interaction between medium and moral foundations - small differences between mediums in all foundatiosn (Cohen's d between .28 and .42).

### Politeness (from Yeomans, Kantor & Tingley, 2018)

```{r Politeness,  fig.align="center", fig.dim=c(12,12), }
library(politeness)

feature_table %>% kable() %>% kable_styling()
...
Pol1 %>%
  ...
  coord_flip() + labs(title = "Proportion of Politeness Terms", 
                      subtitle= "by Medium", y = "Percentage over total WC") +theme(axis.text.x = element_text( 
                           size=18))
...
ggcorrplot(cormat, type = "lower", p.mat = pmat, show.legend = TRUE, lab = TRUE, insig = "blank", lab_size = 5 )



```
**Note**: Some politeness terms were not plotted for not showing any differences among conditions.

## Agreement Analyses

### Dichotomous Outcome - Did you agree with your conversation partner?

For these analyses we considered agreement as the perception of each participant of having reached an agreement with her conversation partner - regardless of their actual dis/agreement.
...
data_Ag %>%
  ggplot(aes(fill = Dich_Agree)) + 
  geom_bar(aes(Topic), position = position_dodge(width = .6)) + 
  geom_bar(aes(Medium), position = position_dodge(width = .6)) + 
  geom_bar(aes(Extension), position = position_dodge(width = .6)) + 
  scale_x_discrete(limits = c("Audio", "Text", "Short", "Long", "Drugs", "Reparations", "Speakers")) + 
  labs(title = "Conditions and Perceived Agreement", 
       x = "Medium                      Extension                         Topic   ", 
       fill = "Perceived Agreement") 

data_Ag %>%
  melt(id.vars =  "Dich_Agree", measure.vars = c("alt_Dehum", "alt_Conf", "alt_Resp")) %>%
  ggplot(aes(variable, value, fill = Dich_Agree)) + 
  stat_summary(geom = "col", position = position_dodge2(width = .6)) + 
  stat_summary(geom = "errorbar", position = position_dodge2(width = .6)) + 
  scale_x_discrete(labels = c("Mind", "Conflictive", "Responsive"))+
  labs(title = "Partner Perception and Perceived Agreement", x = "Partner Perception", fill = "Perceived Agreement") 

```

We ran a generalized linear mixed model to estimate the influence of categories of words used on the likelihood of perceiving agreement. Pair was introduced as a random variable.
...
Mixed<-glmer(data=data_Ag, Agreed ~ function..function.words. + affect..affect. + social..social. + 
              ...
              informal..informal.language. + Topic + (1 | Pair), family = binomial)

sjPlot::tab_model(Mixed,title = "Logistic Linear Mixed Model predicting Agreement",dv.labels = "Perceived Agreement",
                  pred.labels = c("(Intercept)", "Function words", "Affective terms", "Social terms", 
                  "Cognitive Processes", "Biological Processes", "Drive-related words",
                            "Spatio-temporal terms", "Informal language", "Topic:Reparations", "Topic:Speakers"))
```


```{r glm_dich2}

Mixed2<-glmer(data=data_Ag, Agreed ~ cogproc..cognitive.processes..insight..insight.+
                timeorient..time.orientation..focuspresent..present.focus.+
                informal..informal.language..assent..assent. +  (1 | Pair), family = binomial)

sjPlot::tab_model(Mixed2,title = "Word Categories predicting Perceived Agreement",dv.labels = "Perceived Agreement",
                  pred.labels = c("(Intercept)", "Cognitive: Insight", "Present-oriented terms", 
                                  "Informal assentment words")) 


```


### Aggregated Outcome - Actual approach between positions

To explore actual agreement as a result of the converation among pairs, we estimated an `Approach` variable for each pair. Basically, we estimated the absolute distances between both participants' positions before and after the conversation, and then computed the difference between the two time points. 

$$ Approach =  |Attitude^{Pre}_{P1} - Attitude^{Pre}_{P2}| - |Attitude^{Post}_{P1} - Attitude^{Post}_{P2}| $$

In other words, this measures estimates how much both positions got closer, where higher scores mean that both positions got closer.
...
data_Ag %>%
  ...
  scale_x_discrete(limits = c("Audio", "Text", "Short", "Long", "Drugs", "Reparations", "Speakers"))+
labs(title = "Approach estimates, by Conditions", y = "count", fill = "Approach", 
     x = "Medium                    Extension                                  Topic       ")

```


**Comment**: Further analyses with the `Approach` outcome might require to take a dyadic analytical approach to the data, since we have reason to believe that pairs are not two cases in one group, but two participants that are interacting and influencing one another. Therefore, we can assume that there might be some shared variance at the pair-level that a linear mixed model might not capture. 


## Mediation Analyses

### Separate Analyses
We ran mediation analyses for the three partner attribution outcomes (Mind-like , Conflictive and Responsive). Four word categories were tested for mediation between Medium - Audio vs. Text - and outcomes: Assent words, Present-orientation words, Insight words, Moralized words and Politeness features. In the table below we listed the indirect effects of all mediators for all outcomes, run separately.
...
All_Dehum<-'# direct effect
            ...
            Moral ~ a_4*Medium
            alt_Dehum ~ b_4*Moral
            # indirect effect_4 (a_4*b_4)
            indirect_4 := a_4*b_4
            # mediator 5
            Polite ~ a_5*Medium
            alt_Dehum ~ b_5*Polite
            # indirect effect_5 (a_5*b_5)
            indirect_5 := a_5*b_5
            # direct effect (c)
            direct := c
            # total effect
            total := c + (a_1*b_1) + (a_2*b_2) + (a_3*b_3) + (a_4*b_4)+(a_5*b_5)'
...
lay<-matrix(c(1, 0 , #dehum
              ...
              -1, 0) # Medium)
            , ncol = 2, byrow =TRUE)
Hum_labs = c("Humanization", "Present", "Insight","Assent", "Moral","Polite", "Medium")
...
Conf_labs = c("Conflictive", "Present", "Insight","Assent", "Moral", "Polite", "Medium")
...
Resp_labs  = c("Responsiveness", "Present", "Insight","Assent","Moral", "Polite", "Medium ")
